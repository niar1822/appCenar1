...
const bootstrapAdminFromEnv = async () => {
  ...
  if (
    ...
    console.log(
      "No se creo admin inicial: faltan variables ADMIN_* o estan vacias",
    );
    return;
  }
  ...
  if (exists) {
    console.log(
      `Bootstrap admin omitido: ya existe usuario con email o username del admin (${exists._id})`,
    );
    return;
  }
  ...
  try {
    ...
    const mongoUri =
      env === "qa"
        ...
        : env === "production"
          ? cleanEnv(process.env.MONGO_URI_PROD) ||
            cleanEnv(process.env.MONGO_URI)
          : cleanEnv(process.env.MONGO_URI_DEV) ||
            cleanEnv(process.env.MONGO_URI);

    if (!mongoUri) {
      throw new Error(
        `No hay cadena de Mongo configurada para el entorno "${env}"`,
      );
    }
    console.log("APP_ENV:", env);
    console.log("MONGO_URI_PROD existe:", !!process.env.MONGO_URI_PROD);
    console.log(
      "MONGO_URI empieza correctamente:",
      mongoUri.startsWith("mongodb://") ||
        mongoUri.startsWith("mongodb+srv://"),
    );
    console.log("Mongo host:", mongoUri.replace(/\/\/.*@/, "//***:***@"));
